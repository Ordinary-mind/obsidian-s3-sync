...
export function advanceWriterFrontiers(
  ...
  for (const [writerId, writer] of [...byWriter].sort(([left], [right]) => left.localeCompare(right))) {
    ...
    result[writerId] = [...writer.values()]
      ...
      .sort((left, right) => left.sequence.localeCompare(right.sequence) || left.hash.localeCompare(right.hash));
  }
  return result;
}
...
export async function verifyWriterFrontiers(
  ...
  for (const [writerId, anchors] of Object.entries(frontiers).sort(([left], [right]) => left.localeCompare(right))) {
