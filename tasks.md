...
- [x] Bucket Versioning 开启时，重复重试不会无意义地产生大量非当前版本。
...
- [x] 使用规范编码的 repositoryId/channel/logical-key/heads 生成稳定冲突 ID。
- [x] 实现 `.s3-sync-conflicts/<conflict-id>/` 物化并永久排除。
- [x] 为所有 put 头保存可打开副本，为 delete 头保存结构化意图。
- [x] 冲突正文使用 sha256(version-id) 等安全 ASCII 文件名，metadata 保存原路径/Version ID；Windows 非法名和跨平台路径也能完整物化。
- [x] 原路径已有本地内容时保持不动；新客户端只显示确定性候选，不标记胜者。
- [x] 冲突期间编辑成为本地草稿，不自动发布普通覆盖。
- [x] 展示并解决 LocalConcurrentRecord 的 editor/磁盘候选；最终值只继承记录中的原 basisHeads，未选暂存不在提交后立即删除。
- [x] 实现“选本地”“选某版本”“用当前合并内容”“确认删除”。
- [x] 解决命令冻结操作时 observedHeads 和选定字节；已知头变化时要求刷新。
- [x] 未见并发头稍后到达时形成新冲突。
- [x] 冲突集合扩展导致 conflictId 变化时迁移本地草稿，不误清理。
- [x] 文件/目录前缀结构冲突使用稳定跨路径 ID，解决前不自动应用受影响子树。
- [x] 冲突解决传播后清理副本；未提交草稿和恢复文件不得自动删除。

测试：

- [x] 第三个全新客户端得到相同远端冲突集合和全部内容。
- [x] 冲突副本不上传，增加客户端不会产生副本风暴。
- [x] 两设备同时不同方式解决冲突时产生新冲突。
- [x] 解决预览后新增远端头、修改本地草稿和重启均安全。
- [x] 1,025 个以上等价头和冲突头可分步归约；任一步崩溃都不丢失未覆盖头。
...
- [x] 状态栏显示恢复、扫描、拉取、验证、上传、应用、冲突和等待重试。
- [x] 提供“立即同步”“仅预览”“完整校验”“查看 Vault 冲突”。
- [x] “完整校验”展示对象覆盖率、缺失闭包和中断续检；未完成或失败不显示仓库完全健康。
- [x] 展示最后成功拉取、发布、完整审计、pending apply、Outbox、LocalConcurrentRecord、恢复文件/post-capture edit 和提交缺口。
- [x] 每轮展示逐路径决策：相同、本地 put、远端 put、墓碑、冲突、忽略、unknown。
- [x] 错误区分认证、网络、限流、完整性、仓库身份、本地路径和冲突。
- [x] 提供指数退避倒计时和手动重试。
- [x] 导出脱敏诊断包；默认 Hash 化路径，不含凭证和正文。
- [x] 高风险克隆/新世代操作显示 repositoryId、Prefix、对象数量、大小和恢复位置。
- [x] 仓库损坏、anchor 缺失和状态丢失只能进入诊断/重新接入，不提供“一键清空后重传”。
...
- [x] 提供 ConfigProfile 编辑器，声明最低目标 Obsidian 版本，并区分 base、主题、snippets、便携/设备本地插件、插件包和 plugin data。
- [x] profile 变更明确显示“停止管理”与“传播删除”的差异。
- [x] 展示本地/远端 ConfigTree、快照头和逐文件 diff。
- [x] ConfigTree 冲突提供“选本地树”“选远端树”“生成合并树”。
- [x] 插件代码变化展示插件 ID、版本、来源 writer 和兼容性。
- [x] `data.json` 启用前显示明文远端存储和启发式检测局限。
- [x] 配置应用要求显式确认、恢复位置和停用/重载提示。
- [x] 配置 pending、冲突和应用失败不显示为普通网络失败。
...
- [x] 建立 1 万、10 万小文件，大附件和高频配置基准集。
- [x] 验证冻结的 Chunk 上限可让大型 bootstrap 单 Commit 发布且移动端内存受控。
- [x] 限制 Hash、上传、下载并发，记录桌面和移动端内存峰值。
- [x] 大文件流式读取、Hash、暂存和上传；超限路径可解释隔离。
- [x] Blob 存在性缓存失效时回退 GET/Hash，不影响发布正确性。
- [x] 完整审计支持进度、取消和空闲分片；取消不产生删除证据。
- [x] 设计 checkpoint 状态根和 writer frontier 验证；无法验证时不允许新客户端跳过历史。
- [x] checkpoint/latest/device-head 全部是可删除缓存，不参与正确性。
...
- [x] 统计活跃、冲突、历史和孤儿 Blob/Tree/Chunk/Commit 大小。
- [x] 展示去重节省、历史增长和供应商请求成本。
- [x] 识别 Commit 发布前孤儿，但 v1 不自动删除。
- [x] 禁止对全部协议对象使用按 LastModified 自动过期。
- [x] 实现新 repositoryId 世代计划：冻结源计划、新 descriptor 继承旧 current/histories 与参与设备已知历史并集、写新世代、验证树和 Hash、迁移设备、保留旧世代。
- [x] 新增历史排除根与源世代 Vault 头重叠时列出全部受影响版本并阻断，用户迁移/导出前不得从新世代静默省略。
- [x] 新世代迁移或任何维护删除前强制完成源/目标可达对象完整校验。
- [x] 正常设备凭证不需要 DeleteObject；维护删除使用独立权限和二次确认。
- [x] 无锁原地 GC 在真实多设备故障证明前不得实现。
...
- [x] 提供普通同步最小权限 S3 Policy，不包含协议根 DeleteObject。
- [x] 提供可选 probe/维护权限 Policy，限制到独立 Prefix。
- [x] 日志、Notice、错误堆栈和诊断包不泄露 secret、正文或敏感配置值。
- [x] UI 明确 v1 可信写入者假设、路径/字节应用层明文、无签名、无 E2EE 和插件代码执行风险。
- [x] 对所有远端 JSON 执行对象字节、字符串字节、层级、通用/专用数组和 parent 数量限制，并在构造完整对象图前尽早拒绝。
- [x] 对路径穿越、别名、保留目录、超长 Key、畸形 Unicode 和重复 JSON Key 做模糊测试。
- [x] 对固定 Unicode case-fold 表、UTF-8 数组排序和路径段前缀冲突运行跨运行时一致性测试。
- [x] 未知字段、未来协议版本和不规范 JSON 默认拒绝。
- [x] 检查依赖体积、供应链漏洞、许可证和移动端兼容性。
- [x] 审核本地状态目录、插件自身和凭证在自定义 configDir 下的永久排除。
...
- [x] 多客户端确定性模拟器控制提交乱序、重复、晚可见、掉线、重启和所有本地 I/O 边界。
- [x] 自动覆盖 `design.md` 第 21 节全部冻结验收场景。
- [x] 随机运行创建、编辑、删除、rename、离线、重连、冲突解决和配置快照操作。
- [x] 每轮验证：已发布内容可达、无静默胜者、客户端最终头集合一致、pending 不丢失。
- [x] 真实 S3 执行大型 bootstrap、中断续传、第三客户端克隆、仓库世代隔离和 ConfigTree 恢复。
- [ ] 桌面和移动端执行启动、挂起、恢复、插件更新和卸载/重装演练。
- [x] 删除本地操作状态：空 Vault 从远端重建，非空 Vault 强制非破坏性接入。
- [x] 制作协议兼容表和“不支持旧原型原地升级”的发布说明。

最终验收：

- [x] 编辑基线永远来自 projectedHeads。
- [x] Outbox 可逐字节重放，sequence 不跳过或复用。
- [x] 任意本地应用竞态不丢弃字节。
- [x] 任意本地不确定缺失不产生墓碑。
- [x] 两设备并发不会静默覆盖内容、删除意图或完整配置树。
- [x] 第三客户端继承 Vault 和 ConfigTree 冲突。
- [x] 首次接入、状态丢失和新世代迁移默认不覆盖、不删除。
- [x] 用户可手动同步并关闭任意自动调度。
...
- [ ] 完成任务 1 至 4。
- [x] 纯内存三客户端模拟通过。
- [x] 删除本地数据库后可从远端测试向量重建相同状态。

### M2：安全同步内核

- [x] 完成任务 5 至 7。
- [ ] 两台测试 Vault 可同步普通文件、墓碑和离线变化。
- [x] editor-change、Outbox 和本地应用崩溃测试通过。

### M3：Vault 产品闭环

- [x] 完成任务 8、9、11。
- [x] 仓库向导、手动/自动同步、Vault 冲突解决和诊断可用。
- [x] 第三客户端冲突传播验收通过。

### M4：配置快照

- [x] 完成任务 10、12。
- [x] 插件、主题和选定配置以完整树方式预览、冲突、应用和恢复。
- [x] 插件代码、敏感 data.json 和第三方运行期限制有明确交互。

### M5：发布加固

- [ ] 完成任务 13 至 16 中发布必需项。
- [ ] 完成真实供应商、桌面和移动端验证。
- [x] README、协议兼容表、灾难恢复和安全边界与实现一致。
