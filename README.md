...
**v1 协议设计基线已于 2026-07-11 完成文档冻结审查**，但尚无可用于真实数据的稳定版本。提交本次三份指导文档后，可以开始任务 0 的 Schema、固定向量和测试底座编码；任务 0 验收通过前仍不得开始同步核心（任务 1）。
...
任务 0 的机器可读 JSON Schema、固定字节/Hash/Key 测试向量、测试底座和支持矩阵仍未完成；这些产物进入版本控制前不开始核心实现。也不要对真实 Bucket 使用旧版“本地重建远端”流程。
