...
describe("portable ConfigTree compatibility", () => {
  ...
  it("blocks the whole Tree for missing manifests, app version, desktop-only, or local aliases", () => {
    ...
    const result = assessConfigTreeCompatibility({ tree, currentAppVersion: "1.9.0", isDesktop: false, syncPluginId: "obsidian-s3-sync", stagedManifestBytes: new Map(), localPluginManifests: new Map(), localPluginDirectories: ["P"] });
