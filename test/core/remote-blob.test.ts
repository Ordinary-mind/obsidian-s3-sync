...
describe("remote Blob download", () => {
  it("requires both declared size and content address hash", async () => {
    ...
    const store = { getStream: async (requested: string) => { if (requested !== key) throw new Error("unexpected key"); return objectBodyFromBytes(bytes); }, list: async () => ({ keys: [] }), head: async () => ({ size: 2 }), putImmutable: async () => undefined };
