...
- [ ] B 恢复网络并执行“同步”。
- [ ] B 先拉取并停止发布，自动打开冲突窗口。
- [ ] 每个冲突文件默认只占一行：左侧为完整文件名，右侧恰好是“左右对照”“使用本地版本”“使用远端版本”三个文字按钮。
- [ ] 三个主按钮都有准确的悬停说明；主操作区没有含义不明的纯图标按钮，Hash 与远端头位于折叠的技术详情。
- [ ] 点击“左右对照”后，UTF-8 文本完整显示为本地/远端两栏，不出现行级或分段选择按钮。
- [ ] 二进制、无效 UTF-8、超过 1 MiB 或超过 20,000 行的候选只显示无法文本预览的原因和大小，整文件选择仍可用。
- [ ] 只有远端存在多个不同内容时才显示远端版本下拉框；内容相同的并发 put 头合并为一项。
- [ ] 展开“技术详情与诊断”后，可通过文字按钮打开当前本地文件、原本机副本或所选远端副本。
- [ ] 删除候选没有伪造正文。
- [ ] 选择删除结果必须经过二次确认；取消确认后文件和冲突状态不变。
- [ ] 本机已删除、云端已修改时，可以选择“保留本机删除结果”。
- [ ] 选择本地版本或某个远端版本后，冲突变为已解决。
- [ ] 选择远端版本后可从“打开原本机副本”查看经过 Hash/大小校验的恢复副本，当前主文件不被覆盖。
- [ ] 在非首个 writer Commit 的 `durable-confirm` 前模拟中断，重试后只确认已有远端 Commit，不重复发布；自身文件事件不产生额外 push。
- [ ] 留下 `prepared` 或 `recovery-moved` 文件 journal 后点击“同步”，先完成 Outbox 远端确认，再自动打开冲突窗口；选择当前远端候选可续跑，不显示 `apply-journal-recovery` 阻断。
- [ ] 新设备或重建本地 writer 后解决已有远端冲突：首个 writer Commit 带一个父版本时发布为 `change`，带多个冲突头时发布为 `conflict-resolution`，不会出现 `commit-envelope-invalid`。
- [ ] A、B 已同步到同一基线且 B 保持文件打开；A 修改并推送后，B 拉取应自动 fast-forward，插件触发的编辑器刷新不得生成本地 dirty intent、冲突或 `recovery-required` journal。
- [ ] 模拟远端版本已安装但最终对账被打断；在冲突窗口重新选择同一远端版本后应完成对账并清除 journal，不再循环报 `CONFLICT_APPLY_RECOVERY_REQUIRED`。
- [ ] 可复制错误通知约 5 秒后自动关闭；在关闭前复制按钮仍可用。
- [ ] 冲突解决后保留的恢复副本显示为“安全保留”，不计入“需要处理”或“等待恢复或验证”。
- [ ] 冲突窗口底部准确显示可清理恢复文件、已解决冲突目录和受保护副本数量。
- [ ] 关闭冲突窗口后，可从状态页“本地安全副本”或展开后的“管理本地副本”重新进入清理界面。
- [ ] 点击“清理已解决副本”必须二次确认；清理后当前 Vault 文件、S3 对象和历史不变，`.obsidian-s3-sync-local` 管理目录仍保留。
- [ ] 未解决冲突、活动 journal、共享引用、配置恢复以及被修改过的恢复副本均不会被批量清理；中断后再次清理可以续跑。
- [ ] A、B 再同步，最终只剩一个解析头。
